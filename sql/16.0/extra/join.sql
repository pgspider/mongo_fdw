...
--Testcase 313:
CREATE FOREIGN TABLE temp_t1 (a int, b int)
 SERVER mongo_server OPTIONS (database 'join_regress', collection 'temp_t1');

--Testcase 314:
CREATE FOREIGN TABLE temp_t2 (a int, b int)
 SERVER mongo_server OPTIONS (database 'join_regress', collection 'temp_t2');

--Testcase 315:
CREATE FOREIGN TABLE temp_t3 (x int, y int)
 SERVER mongo_server OPTIONS (database 'join_regress', collection 'temp_t3');

--Testcase 316:
DELETE FROM temp_t3 USING temp_t1 table1 WHERE temp_t3.x = table1.a;
--Testcase 317:
SELECT * FROM temp_t3;
--Testcase 318:
DELETE FROM temp_t3 USING temp_t1 JOIN temp_t2 USING (a) WHERE temp_t3.x > temp_t1.a;
--Testcase 319:
SELECT * FROM temp_t3;
--Testcase 320:
DELETE FROM temp_t3 USING temp_t3 t3_other WHERE temp_t3.x = t3_other.x AND temp_t3.y = t3_other.y;
--Testcase 321:
SELECT * FROM temp_t3;
...
--Testcase 322:
select temp_t1.*, temp_t2.*, unnamed_join.* from
  temp_t1 join temp_t2 on (temp_t1.a = temp_t2.a), temp_t3 as unnamed_join
  for update of unnamed_join;

--Testcase 323:
select foo.*, unnamed_join.* from
  temp_t1 join temp_t2 using (a) as foo, temp_t3 as unnamed_join
  for update of unnamed_join;
...
select bar.*, unnamed_join.* from
  (temp_t1 join temp_t2 using (a) as foo) as bar, temp_t3 as unnamed_join
  for update of foo;
...
explain (costs off)
select a1.id from
  (a1 a1 left join a1 a2 on true)
  left join
  (a1 a3 left join a1 a4 on a3.id = a4.id)
  on a2.id = a3.id;
--Testcase 364:
explain (costs off)
select a1.id from
  (a1 a1 left join a1 a2 on a1.id = a2.id)
  left join
  (a1 a3 left join a1 a4 on a3.id = a4.id)
  on a2.id = a3.id;
...
explain (costs off)
select 1 from a1 t1
    left join a1 t2 on true
   inner join a1 t3 on true
    left join a1 t4 on t2.id = t4.id and t2.id = t3.id;
...
explain (costs off)
select ss1.x
from (select f1/2 as x from int4_tbl i4 left join a1 on a1.id = i4.f1) ss1
...
explain (costs off)
select 1 from a1 t1
  left join (a1 t2 left join a1 t3 on t2.id = 1) on t2.id = 1;
...
--Testcase 373:
explain (costs off)
select c1.id, ss.a from c1
  left join (select d1.a from onerow, d1 left join b1 on d1.a = b1.id) ss
  on c1.id = ss.a;
...
--Testcase 374:
explain (costs off)
select a1.* from a1 left join parted_b pb on a1.b_id = pb.id;
...
-- Can not create unique index. This operation is not supported for foreign tables.
-- -- test more complex permutations of unique joins
-- create foreign table j1 (_id name, id1 int, id2 int) server mongo_server options (database 'join_regress', collection 'j1');
-- insert into j1 values('0',1,1),('1',1,2);
